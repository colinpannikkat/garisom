...
# Global auth token for earth access
token = None

# URL Constants
NOAA_URL = "https://gml.noaa.gov/webdata/ccgg/trends/co2/co2_annmean_gl.csv"
EARTHDATA_URL = "https://api.giovanni.earthdata.nasa.gov/timeseries"
    ...
    for y in range(minyr, maxyr + 1):
        ...
        for _, group in x.loc[x['Year'] == y].groupby("Day"):
            ...
            if (first):
                ...
                ttime_dt = pd.concat(
                    [DataFrame([[y, day, ttime]], columns=ttime_dt.columns), ttime_dt],
                    ignore_index=True
                )
...
def _fit_lines(xday, yt_time, show_plt=False):
    ...
    if reg1 is None or reg2 is None or reg3 is None:
        raise Exception("Atleast one regression failed.")
...
def _convert_time_to_utc(start_date, end_date, latitude, longitude):
    ...
    Outputs:
    - `start_date_utc`: Start date in UTC time in the format 'YYYY-MM-DDThh:mm:ss'
    - `end_date_utc`: End date in UTC time in the format 'YYYY-MM-DDThh:mm:ss'
    '''
    tf = TimezoneFinder()
    ...
    start_date_local = local_tz.localize(datetime.strptime(start_date, "%Y-%m-%dT%H:%M:%S"))
    end_date_local = local_tz.localize(datetime.strptime(end_date, "%Y-%m-%dT%H:%M:%S"))
...
def _get_time_series(start_date, end_date, latitude, longitude, variable, version):
    """
    Calls the Giovanni Time Series service to get time series data.

    Inputs:
    - `start_date`: Start of time series in YYYY-MM-DDThh:mm:ss format (UTC)
    - `end_date`: End of the time series in YYYY-MM-DDThh:mm:ss format (UTC)
    - `latitude`: Latitude of the location
    - `longitude`: Longitude of the location
    ...
    headers = {
        'Authorization': f'Bearer {token}'  # token is global
    }
    ...
    for attempt in range(retries):
        try:
            r = session.get(base_url, params=query_parameters, headers=headers, timeout=10)
    ...
    try:
        raise Exception(f"Error code {r.status_code} from url {base_url} : {r.text}")
    except Exception:
        raise Exception(f"Could not connect to {base_url}. Please try again later.")


def _parse_time_series(ts_str, conv_func: Callable | None = None):
    """
...
def _calc_wind_from_nldas(start_date, end_date, latitude, longitude):
    ...
    x_wind = _parse_time_series(
                _get_time_series(
                    ...
                    latitude=latitude,
                    longitude=longitude,
                    variable="NLDAS_FORA0125_H_2_0_Wind_E",
                    version="2.0"
                )
            )[1]['data'].fillna(0)
    ...
    y_wind = _parse_time_series(
                _get_time_series(
                    ...
                    latitude=latitude,
                    longitude=longitude,
                    variable="NLDAS_FORA0125_H_2_0_Wind_N",
                    version="2.0"
                )
            )[1]['data'].fillna(0)
...
def _calc_vpd_from_nldas(start_date, end_date, latitude, longitude):
    ...
    tair = _parse_time_series(
        _get_time_series(
                ...
                latitude=latitude,
                longitude=longitude,
                variable="NLDAS_FORA0125_H_2_0_Tair",
                version="2.0"
    ...
    shum = _parse_time_series(
        _get_time_series(
                ...
                latitude=latitude,
                longitude=longitude,
                variable="NLDAS_FORA0125_H_2_0_Qair",
                version="2.0"
        )
    )[1]['data']
    ...
    pair = _parse_time_series(
        _get_time_series(
                ...
                latitude=latitude,
                longitude=longitude,
                variable="NLDAS_FORA0125_H_2_0_PSurf",
                version="2.0"
...
def readin_nldas_data(climate_data: list[tuple[DataFrame, str]],
                      missing_vars: list[str], lat: DataFrame, lon: DataFrame):
    '''
    This function retrieves weather data for a site from the NASA GES DISC Giovanni Time Series Service API
    and fills in any missing data.

    Information on data can be found here: https://disc.gsfc.nasa.gov/information/documents?title=Giovanni%20In%20The%20Cloud:%20Time%20Series%20Service
    ...
    variables = {
        "Solar_Wm2": ("NLDAS_FORA0125_H_2_0_SWdown", "2.0", None),
        "Rain_mm": ("NLDAS_FORA0125_H_2_0_Rainf", "2.0", None),
        "Wind_ms.1": ("", "", None),  # see calc_wind_from_nldas
        "Tair_C": ("NLDAS_FORA0125_H_2_0_Tair", "2.0", lambda x: x - 273.15),  # convert K to C
        "Tsoil_C": ("NLDAS_NOAH0125_H_2_0_SoilT_0_10cm", "2.0", lambda x: x - 273.15),  # convert K to C
    ...
    for i, site_missing_vars in enumerate(tqdm(missing_vars, desc="Sites")):
        ...
        start_date = datetime.strptime(f"{int(c_data.iloc[0]['Year'])}-{int(c_data.iloc[0]['Day'])}-{int(c_data.iloc[0]['Hour'])}", "%Y-%j-%H").strftime("%Y-%m-%dT%H:%M:%S")
        if c_data.iloc[-1]['Day'] == 366:  # leap year, for some reason datetime strptime can't handle 366 day
            end_date = (datetime(int(c_data.iloc[-1]['Year']), 1, 1) + timedelta(days=int(c_data.iloc[-1]['Day']) - 1,
                        hours=int(c_data.iloc[-1]['Hour']))).strftime("%Y-%m-%dT%H:%M:%S")
        else:
            end_date = datetime.strptime(f"{int(c_data.iloc[-1]['Year'])}-{int(c_data.iloc[-1]['Day'])}-{int(c_data.iloc[-1]['Hour'])}", "%Y-%j-%H").strftime("%Y-%m-%dT%H:%M:%S")
        ...
        for var in tqdm(site_missing_vars, leave=False, desc="Missing vars"):
            ...
            if var not in variables:
                ...
                variable, version, conv_func = variables[var]
            ...
            # Add data to climate_data dataframe, technically was collected in terms of UTC time, but the rows
            # *should* line-up
...
def build_config_param_files(data: DataFrame, out_data_path: str) -> None:
    ...
    abs_nir = 0.2           # Absorption of near infared
    abs_par = 0.8           # Absorption of photosynthetically active radiation
    abs_solar = 0.5         # Ground absorption of solar radiation
    ...
    for site, group in site_param_data:
        ...
        df_param['i_latitude'] = group['latitude'].astype(float)
        df_param['i_longitude'] = group['longitude'].astype(float)
        df_param['i_elevation'] = group['elevation'].astype(float)
        df_param['i_slopeI'] = group['slopeI'].astype(float)
        df_param['i_slopeA'] = group['slopeA'].astype(float)
        df_param['i_gWaterP'] = group['gWaterP'].fillna(ground_water_p).astype(float)
        df_param['i_gWaterDist'] = group['gWaterDist'].fillna(ground_water_d).astype(float)
        df_param['i_atmTrans'] = group['atmTrans'].fillna(atm_trans).astype(float)
        df_param['i_solarNoon'] = 0.0  # _solar_noon_correction(df_param['i_longitude']), your mileage may vary
        df_param['i_emiss'] = group['emiss'].fillna(emiss).astype(float)
        df_param['i_absNIR'] = group['absNIR'].fillna(abs_nir).astype(float)
        df_param['i_absPAR'] = group['absPAR'].fillna(abs_par).astype(float)
        df_param['i_absSOLAR'] = group['absSOLAR'].fillna(abs_solar).astype(float)
        df_param['i_emiss'] = group['emiss'].fillna(emiss).astype(float)
        df_param['i_emiss'] = group['emiss'].fillna(emiss).astype(float)
        df_param['i_co2AmbPPM'] = group['co2AmbPPM'].fillna(co2).astype(float)
        df_param['i_layers'] = group['layers'].fillna(soil_layers).astype(float)
        df_param['i_fieldCapFrac'] = group['fieldCapFrac'].fillna(field_cap_frac).astype(float)
        df_param['i_fieldCapPercInit'] = group['fieldCapPercInit'].fillna(field_cap_init).astype(float)
        df_param['i_rockFrac'] = group['rockFrac'].astype(float)
        df_param['i_soilAbsSol'] = group['soilAbsSol'].fillna(soil_abs_sol).astype(float)
        df_param['i_rhizoPer'] = group['rhizoPer'].fillna(rhizo_per).astype(float)
        df_param['i_texture'] = group['texture']
        df_param['i_baperga'] = group['baPerGa'].astype(float)
        df_param['i_leafAreaIndex'] = group['leafAreaIndex'].astype(float)
        df_param['i_soilXHeight'] = group['soilXHeight'].fillna(soil_Xheight).astype(float)
        df_param['i_height'] = group['height'].astype(float)
        df_param['i_leafPerBasal'] = group['leafPerBasal'].astype(float)
        df_param['i_leafWidth'] = group['leafWidth'].fillna(leaf_width).astype(float)
        df_param['i_leafAngleParam'] = group['leafAngleParam'].fillna(leaf_angle).astype(float)
        df_param['i_aspect'] = group['aspect'].fillna(root_aspect).astype(float)
        df_param['i_rootBeta'] = group['rootBeta'].fillna(root_beta).astype(float)
        df_param['i_leafPercRes'] = group['leafPercRes'].fillna(leaf_per).astype(float)
        df_param['i_kmaxTree'] = group['kmax_tree'].fillna(kmax).astype(float)
        df_param['i_pinc'] = group['p_inc'].fillna(p_inc).astype(float)
        df_param['i_LSC'] = group['lsc'].fillna(lsc).astype(float)
        df_param['i_LSCpref'] = group['lsc_pref'].fillna(lsc_pref).astype(float)
        df_param['i_cr'] = group['cr'].astype(float)
        df_param['i_br'] = group['br'].astype(float)
        df_param['i_cs'] = group['cs'].astype(float)
        df_param['i_bs'] = group['bs'].astype(float)
        df_param['i_cl'] = group['cl'].astype(float)
        df_param['i_bl'] = group['bl'].astype(float)
        df_param['i_sapwoodT'] = group['sapwoodT'].astype(float)
        df_param['i_conduitDiam'] = group['conduitDiam'].astype(float)
        df_param['i_qMax'] = group['qMax'].fillna(q_max).astype(float)
        df_param['i_vmax25'] = group['vmax25'].astype(float)
        df_param['i_jmax25'] = group['jmax25'].astype(float)
        df_param['i_kc25'] = group['kc25'].astype(float)
        df_param['i_ko25'] = group['ko25'].astype(float)
        df_param['i_comp25'] = group['comp25'].astype(float)
        df_param['i_thetaC'] = group['thetaC'].astype(float)
        df_param['i_havmax'] = group['havmax'].astype(float)
        df_param['i_hdvmax'] = group['hdvmax'].astype(float)
        df_param['i_svvmax'] = group['svvmax'].astype(float)
        df_param['i_lightCurv'] = group['lightCurv'].fillna(light_curv).astype(float)
        df_param['i_lightComp'] = group['lightComp'].astype(float)
        df_param['i_hajmax'] = group['hajmax'].astype(float)
        df_param['i_hdjmax'] = group['hdjmax'].astype(float)
        df_param['i_svjmax'] = group['svjmax'].astype(float)
...
def main():
    ...
    noaa_data = readin_noaa_data(NOAA_URL)
